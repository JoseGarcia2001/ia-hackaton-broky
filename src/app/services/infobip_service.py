...
from ..models.whatsapp import WhatsAppResponse, WhatsAppError
...
class InfobipService:
    ...
    async def send_text_message(self, to: str, text: str) -> WhatsAppResponse:
        ...
        try:
            message_data = {
                "messages": [
                    {"from": self.whatsapp_from, "to": to, "content": {"text": text}}
                ]
            }
    ...
    def _extract_message_content(
        self, message_data: Dict[str, Any], message_type: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if message_type == "text":
                ...
                content = {"text": message_data.get("text", ""), "type": "text"}
                ...
                content = {"raw_data": message_data, "type": "unsupported"}
    ...
    async def download_metadata(self, image_url: str) -> bytes:
        """
        Download metadata from a URL
        """
        try:
            async with httpx.AsyncClient() as client:
            ...
            logger.error(f"Error downloading metadata from {image_url}: {str(e)}")
            raise
